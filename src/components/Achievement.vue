...
<template>
  <div class="image-div" :class="`level-${level} ${level === maxLevels ? (maxLevels === 1 ? 'level-solo' : 'level-last') : ''} ${level === maxLevels - 1 ? 'level-last-prev' : ''} ${special ? 'level-special' : ''} ${noLevel ? 'no-level' : ''}`">
    ...
    <AchievementAvatar :no-border-radius="special" class="image" alt="achievement" :image-id="imageId" :border-width="special ? '0px' : '2px'" border-offset="2px" :size="size" :size-mobile="sizeMobile"></AchievementAvatar>
  </div>
</template>
